...
import { parseUserReply, generateAlphaResponse, generateConversation, generateUserSummary, parseOnboardingReply, onboardingConversation, AIFailureError, type EmailMessage } from "../../../lib/ai";
import { 
  verifyResendWebhook, 
  checkRateLimit, 
  getClientIP, 
...
async function handleOnboardingReply(
  ...
  try {
    // Fetch conversation history so Alpha remembers previous messages
    ...
    const conversationHistory = (priorEmails || []) as EmailMessage[];

    // Use conversational onboarding - Alpha naturally chats to collect name + goal
    const result = await onboardingConversation(userMessage, conversationHistory);
    ...
    if (!result.complete) {
      // Alpha is still chatting, collecting info naturally
      const safeReply = escapeHtml(result.reply);
      const responseHtml = wrapEmailHtml(`
        <p style="font-size: 16px; line-height: 1.6; margin-bottom: 16px; white-space: pre-wrap;">${safeReply}</p>
        <p style="font-size: 16px; color: #6b7280;">- alpha</p>
      `);
      ...
      await sendEmail({
        ...
        subject: replySubject,
        html: responseHtml,
        text: wrapEmailText(`${result.reply}\n\n- alpha`),
      });

      await supabase.from("emails").insert({
        user_id: userId, direction: "outbound", subject: replySubject, content: result.reply,
      });

      return new Response(JSON.stringify({ success: true, action: "onboarding_conversation" }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      });
    }

    // Onboarding complete - we have both name and goal
    const firstName = result.name!;
    const goalDescription = result.goal!;
    ...
    // Send Alpha's welcome reply (generated by AI)
    const safeReply = escapeHtml(result.reply);
    ...
    // Send fallback
    const fallbackText = "hey! i got your message but had a little trouble on my end. mind sending that again?";
    const responseHtml = wrapEmailHtml(`
